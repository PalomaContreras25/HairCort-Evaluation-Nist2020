...
print(file.path(data_path, "layout_wells_test7_081025.csv"))  
data <- read.csv(file.path(data_path, "raw.plate.csv"))
layout <- read.csv(file.path(data_path,"layout_wells_test7_081025.csv"), 
...
m$alpha <- 1
m$alpha <- ifelse(m$Category == "Std", 2, 0.8)


levels(m$Category)[c(1,2,3,6)] <-"Ref_value"
...
m_ave_std <- subset(m_ave, Category == "Std")
m_ave_stdB <- subset(m_ave, Category == "Std-B")


```

```{r plot1.paloma, warning = FALSE, echo = FALSE}
a <- filter(m, Category == "Std-B" | Category == "P10" | Category == "P20", !is.na(Category))
a <- a[order(a$Sample),]
...
ggplot(a, aes(x = Conc_pg.ml, 
  ...
  geom_hline(yintercept = 20, linetype = "dashed", 
             color = "orange", linewidth = 0.8) +   
  labs(title = "Fig1. Only dilution of Standard and Paloma samples",
             subtitle = "Raw vals",
...
ggplot(a, aes(x = Sample, 
  ...
  geom_hline(yintercept = 20, linetype = "dashed", 
             color = "orange", linewidth = 0.8) +   
  labs(title = "Fig2. Only dilution of Standard and Paloma samples",
             subtitle = "Raw vals",
             x = "Sample ID", 
             y = "Binding Percentage") +
  scale_y_continuous(n.breaks = 10) 
