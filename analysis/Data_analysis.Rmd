...
```{r all.groups.binding.perc.sample, fig.width=8, fig.height=6, echo = FALSE}
...
```{r goodqual.groups.binding.perc.sample, fig.width=8, fig.height=6, echo = FALSE}
...
## Binding Percentages vs Hair Count (if information exists)

```{r bind.perc.hair.count, fig.width=8, fig.height=6, echo = FALSE, echo = FALSE}
...
#  geom_text(aes(label = Sample), 
 #           size = 2.5, vjust = 0, hjust = -0.2, color = "gray40") +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.3, alpha = 0.09, 
              aes(group = Category)) +
  geom_hline(yintercept = 80, linetype = "dashed", color = "gray", linewidth = 0.8) +
  geom_hline(yintercept = 20, linetype = "dashed", color = "gray", linewidth = 0.8) +
  labs(title = "Binding % by Sample",
       subtitle = "Non-corrected values (for weight, dilution)",
       x = "Hair Count", y = "Binding Percentage") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(n.breaks = 10) +
  scale_x_continuous(n.breaks = 10)
} else {
    message(paste("Skipping plot, data set is missing information on 
