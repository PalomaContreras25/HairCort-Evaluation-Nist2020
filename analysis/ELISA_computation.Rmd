...
layout <- read.csv("./data/layout_wells_test3_091524.csv", 
...
results <- read.csv("./data/myassays_table_test3_091524.csv", 
                    stringsAsFactors = TRUE, 
                    na.strings = c("", " ", "-"))
results <- results %>% 
  select(!Sample) 
...
info <- read.csv("./data/sample_info_test3_091524.csv", 
                 stringsAsFactors = TRUE, 
                 na.strings = c("", " "))
kable(head(info)) 
...
write.csv(data.flagged, "./data/Data_QC_flagged.csv", 
          row.names = FALSE) 

# table with samples that failed
failed_samples <- data.flagged[!is.na(data.flagged$Failed_samples),]
write.csv(failed_samples, "./data/failed_samples.csv", 
          row.names = FALSE) 
nrow(failed_samples)
...
data.no_failed <- m2 %>% 
 ...
 select(-CV_categ, -Binding.Perc_categ)
 
write.csv(data.no_failed, "./data/Data_QC_filtered.csv", row.names = FALSE) 
```
...
data <- df %>% 
  ...
  filter(Binding.Perc < 80 & Binding.Perc > 20) %>%
    select(Sample, Binding.Perc, Weight_mg, Buffer_ml, Spike, Ave_Conc_ug.dL) 
...
data$Buffer_ml <- replace(data$Buffer_ml, data$Buffer_ml == 0.06, '60 uL')
data$Buffer_ml <- replace(data$Buffer_ml, data$Buffer_ml == 0.25, '250 uL')
...
dSpike$Buffer_ml <- replace(dSpike$Buffer_ml, dSpike$Buffer_ml == 0.06, '60 uL')
dSpike$Buffer_ml <- replace(dSpike$Buffer_ml, dSpike$Buffer_ml == 0.25, '250 uL')
