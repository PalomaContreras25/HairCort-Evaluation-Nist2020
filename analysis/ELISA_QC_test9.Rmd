...
results <- read.csv(file.path(data_path,"myassays_table_test9_251008.indiv.calc.csv"), 
                    stringsAsFactors = TRUE, 
                    na.strings = c("", " ", "-"))
...
colnames(m) <- c("Wells", "Sample", "Category", 
                ...
                "Extraction_ratio", "Sub_categ", "Order", "Num_strands", "Person",   "Dilution_sample", "Raw.OD",  
...
m <- m[order(m$Sample),]
kable(m[c(9, 10, 11, 12, 13, 15, 23, 26), ]) 
...
print(out_curve[2])
#kable(out_curve[,c("Sample", "Category", "Weight_mg", "Buffer_nl", "Dilution_sample", "Raw.OD", "Binding.Perc", "Conc_pg.ml", "Ave_Conc_pg.ml", "CV.Perc", "SD", "CV_categ", "Binding.Perc_categ")])
...
write.csv(data.flagged, file.path(data_path, "Data_QC_flagged2.csv"), 
          row.names = FALSE) 


# table with samples that failed
failed_samples <- data.flagged[!is.na(data.flagged$Failed_samples),]
write.csv(failed_samples, file.path(data_path, "failed_samples2.csv"), 
          row.names = FALSE) 
...
write.csv(data.no_failed, file.path(data_path, "Data_QC_filtered2.csv"), row.names = FALSE) 
