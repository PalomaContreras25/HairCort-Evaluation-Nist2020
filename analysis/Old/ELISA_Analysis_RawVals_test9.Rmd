...
data2 <- read.csv(file.path(data_path, "Data_QC_flagged2.csv"))
...
a <- filter(data_std, Category == "Spike" | Category == "Std" | Category == "P10" | Category == "P10-SP", !is.na(Binding.Perc))
a$Sample_num <- 1:length(a$Sample)
a <- a[order(a$Order), ]

ggplot(a, aes(x = fct_reorder(Sample, Order), 
                 y = Binding.Perc, color = Category)) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 0.5, alpha = 0.1) +
    geom_errorbar(aes(ymin = Binding.Perc , ymax = Binding.Perc , color = Category, alpha = 0.05)) +
  geom_point(size = 2) +
  geom_text(aes(label = Sample), size = 3, vjust = -1, hjust = 0.5, color = "gray30") +
  geom_hline(yintercept = 80, linetype = "dashed", 
             color = "gray", linewidth = 1) +  
...
## individual calculations (not averaging by duplicate/triplicate)

```{r p10}
data2_std <- join(std, data2, by = "Wells", type = "full")
data2_std <- data2_std[order(data2_std$Order), ]


a <- filter(data2_std, Category == "Spike" | Category == "Std" | Category == "P10" | Category == "P10-SP", !is.na(Binding.Perc))
a$Sample_num <- 1:length(a$Sample)
a <- a[order(a$Order), ]
...
#  stat_poly_eq(aes(x = Sample_num, label = paste(..rr.label.., sep = "~~~")),
 #              formula = y ~ x, parse = TRUE, size = 4) +
 # geom_abline(slope = 14.72, intercept = -3.6, linetype = "dashed", color = "lightblue") + # ideal line
  labs(title = "Observed binding %, Paloma 10mg",
       subtitle = "individual vals. not averaging replicates",
       x = "Sample", y = "Binding Percentage") +
  scale_y_continuous(n.breaks = 10, limits = c(0,100)) + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
a <- filter(data2_std, Category == "Spike" | Category == "Std" | Category == "P20" | Category == "P20-SP", !is.na(Binding.Perc))
a$Sample_num <- 1:length(a$Sample)
a <- a[order(a$Order), ]
...
#  stat_poly_eq(aes(x = Sample_num, label = paste(..rr.label.., sep = "~~~")),
 #              formula = y ~ x, parse = TRUE, size = 4) +
 # geom_abline(slope = 14.72, intercept = -3.6, linetype = "dashed", color = "lightblue") + # ideal line
  labs(title = "Observed binding %, Paloma 20mg",
       subtitle = "individual vals. not averaging replicates",
       x = "Sample", y = "Binding Percentage") +
  scale_y_continuous(n.breaks = 10, limits = c(0,100)) + 
  theme_minimal() +
...
a <- filter(data2_std, Category == "PC-precision" | Category == "Std", !is.na(Binding.Perc))
a$Sample_num <- 1:length(a$Sample)
a <- a[order(a$Order), ]
...
#  stat_poly_eq(aes(x = Sample_num, label = paste(..rr.label.., sep = "~~~")),
 #              formula = y ~ x, parse = TRUE, size = 4) +
 # geom_abline(slope = 14.72, intercept = -3.6, linetype = "dashed", color = "lightblue") + # ideal line
  labs(title = "Observed binding %, Paloma 20mg, Precision",
       subtitle = "individual vals. not averaging replicates",
       x = "Sample", y = "Binding Percentage") +
  scale_y_continuous(n.breaks = 10, limits = c(0,100)) + 
  theme_minimal() +
