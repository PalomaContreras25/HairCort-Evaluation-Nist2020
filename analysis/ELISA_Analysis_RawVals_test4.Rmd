...
data1 <- data[,c("Sample","Wells", "Category", "Binding.Perc", "Ave_Conc_pg.ml", "Weight_mg", "Buffer_nl", "Spike", "Conc_pg.ml")]
...
```{r expABCD.binding.perc, fig.width=10, fig.height=7}
...
ggplot(a, aes(x = Sample_num, 
  ...
  geom_smooth(method = "lm", formula = y ~ poly(x, 3), se = TRUE, linewidth = 0.5, alpha = 0.3) +
  geom_point(size = 2) +
 ...
 # geom_abline(slope = 14.72, intercept = -3.6, linetype = "dashed", color = "lightblue") + # ideal line
  labs(title = "All serial dilutions plus standard: observed binding %",
...
ggplot(exp, aes(x = Sample_num, 
                 y = Binding.Perc, color = Category)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 1), se = TRUE, linewidth = 0.5, alpha = 0.3) +
  geom_point(size = 2) +
  geom_text(aes(label = Sample), size = 3, vjust = -1, hjust = 0.5, color = "gray30") +
  geom_hline(yintercept = 80, linetype = "dashed", 
             color = "gray", linewidth = 1) +  
 ...
 # geom_abline(slope = 14.72, intercept = -3.6, linetype = "dashed", color = "lightblue") + # ideal line
  labs(title = "All serial dilutions plus standard: observed binding %",
       subtitle = "removed high CV samples, linear model",
       x = "Sample", y = "Binding Percentage") +
  scale_y_continuous(n.breaks = 10) + 
  theme_minimal() 
...
## remove TC7
exp <- filter(exp, Sample != c("TD7","TD6"))
...
ggplot(exp, aes(x = Sample, 
  ...
  theme_minimal() +
  annotate("text", x = 1, y = 35, label = paste0("CV: ", cv_values$CV_BP[cv_values$Group == "TP1"], "%"), size = 4) +
  annotate("text", x = 4, y = 35, label = paste0("CV: ", cv_values$CV_BP[cv_values$Group == "TP2"], "%"), size = 4) +
  annotate("text", x = 7, y = 35, label = paste0("CV: ", cv_values$CV_BP[cv_values$Group == "TP3"], "%"), size = 4) +
  annotate("text", x = 1, y = 40, label = paste0("mean: ", round(cv_values$mean_BP[cv_values$Group == "TP1"],2)), size = 4) +
  annotate("text", x = 4, y = 40, label = paste0("mean: ", round(cv_values$mean_BP[cv_values$Group == "TP2"],2)), size = 4) +
  annotate("text", x = 7, y = 40, label = paste0("mean: ", round(cv_values$mean_BP[cv_values$Group == "TP3"],2)), size = 4) 
...
## Experiment E using raw conc values and filtered
```{r expE-concvals-filtered}
# Scatter plot of Experiment D, observed binding % 
...
## Experiment E: Final cortisol vals A (controlling for dilution and weight)
...
## Experiment E: Final cortisol vals B (controlling for dilution, weight and spike)
