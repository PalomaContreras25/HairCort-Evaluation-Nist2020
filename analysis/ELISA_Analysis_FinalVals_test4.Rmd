...
a <- cort_valsA[!(cort_valsA$Sample == "NSB"),]
a$Expected <- NA
...
```{r expABCD.Final_conc_pg.mg_B_allData, include = FALSE}
invisible({capture.output({
...
```{r expABCDfilt.Final_conc_pgB_filt.mg, fig.width=8, fig.height=6}
exp <- a %>%
  filter(!Sample %in% c("TA6","TA7","TB7","TC7", "TC6","TC5", "A2"))
...
```{r expABCD.conc.expect.vs.observed_D}
# remove NSB
data <- cort_valsD[!(cort_valsD$Sample == "NSB"),]
## Expected vals B
