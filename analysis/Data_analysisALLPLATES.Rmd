...
output:  
  workflowr::wflow_html:
    ...
    smooth_scroll: true
params:
  plate: "Test7"        # one of: Test3 ... Test10
...
```
```{r  hair.vs.bindning-facet, fig.width=8, fig.height=6, echo = FALSE}
...
```
```{r  hair.vs.weight-facet, fig.width=8, fig.height=6, echo = FALSE}
...
if ("Num_strands" %in% names(data) && any(data$Num_strands != 0, na.rm = TRUE)) {
data$cortxstrand <- data$Final_pg_mg / data$Num_strands
} else {
...
### Results Formula B
```{r goodqual.ave.cort.vals, fig.width=8, fig.height=6, echo = FALSE, warning = FALSE}
...
if ("Num_strands" %in% names(data) && any(data$Num_strands != 0, na.rm = TRUE)) {
ggplot(data_clean, aes(y = cortxstrand, 
                  x = Sample,
                  color = Category,
                  fill= Category)) +
